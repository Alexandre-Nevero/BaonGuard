...
// ── Helpers (kept from stellar.js for display purposes) ───────────────────────
const STROOP   = 10_000_000;
const PHP_RATE = 58;
const stroopsToUsdc = (s) => Number(s) / STROOP;
const usdcToPhp     = (u) => (u * PHP_RATE).toFixed(2);
const CONFIG = {
...
function ParentDashboard({ wallet, onVaultLoad }) {
  ...
  const fetchVault = useCallback(async () => {
    try {
      setLoading(true);
      const raw = await getVaultInfo();
      // Normalise api.js response shape to match existing UI expectations
      const info = {
        student:        raw.student_address,
        dailyLimit:     raw.daily_limit,
        lastWithdrawal: raw.last_withdrawal_timestamp,
  ...
  const handleInitialize = async (e) => {
    ...
    try {
      ...
      const dailyLimitStroops = Math.round(parseFloat(dailyLimit) * 10_000_000);
      const result = await initializeVault(studentAddr.trim(), import.meta.env.VITE_USDC_TOKEN || "", dailyLimitStroops);
      setTxResult({ type: "success", message: "Vault initialized successfully!", hash: result.tx_hash });
      await fetchVault();
    } catch (err) {
      setTxResult({ type: "error", message: err.message });
    } finally {
  ...
  const handleDeposit = async (e) => {
    e.preventDefault();
    clearTx();
    // Deposit is a direct token transfer — not proxied through our backend.
    // Show a message directing the parent to use Stellar Laboratory or CLI.
    setTxResult({ type: "error", message: "Direct deposits are not supported via the web UI. Use Stellar Laboratory or soroban-cli to transfer USDC to the vault contract." });
  };
    ...
    <div className="animate-in" style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      <div className="card" style={{ borderColor: "var(--surface-3)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "0.75rem", fontSize: "0.82rem" }}>
          {[
            { label: "Contract ID", value: import.meta.env.VITE_CONTRACT_ID || "—" },
            { label: "USDC Token", value: import.meta.env.VITE_USDC_TOKEN || "—" },
            { label: "Network", value: "Testnet" },
            { label: "RPC", value: "https://soroban-testnet.stellar.org" },
          ].map((r) => (
            <div key={r.label}>
...
function StudentDashboard({ wallet }) {
  ...
  const handleWithdraw = async (e) => {
    ...
    try {
      ...
      const amountStroops = Math.round(amt * 10_000_000);
      const result = await withdrawFromVault(wallet, amountStroops);
      setTxResult({ type: "success", message: `Withdrawn ${amt} USDC — enjoy your baon! 🎉`, hash: result.tx_hash });
      setWithdrawAmt("");
      await fetchVault();
    } catch (err) {
      setTxResult({ type: "error", message: err.message });
